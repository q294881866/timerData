...
func Timestamp(now time.Time) string {
	...
	return string([]rune(nowString)[:index])
}

// 数据生产器
func DataProducter() Data {
	data := Data{}
	now := time.Now()
	// 时间戳
	...
	// 今天开机时间 超过一天否
	//	if int64(now.Nanosecond()-startTime.Nanosecond()) > DayTime {
	//		startToday = int64(now.Nanosecond()) - time.Date(now.Year(), now.Month(), now.Day(), 0, 0, 0, 0, time.Local).UnixNano()
	//	} else {
	//		startToday = int64(now.Nanosecond() - startTime.Nanosecond())
	//	}
	...
	// 机器运行信息 5000台机器
	RestTime += float64(0.00000001) * float64(randomNum(0, 4, 2))
...
func task(dater *time.Ticker, datas []string) {
	go func() {
		count := 0
		length := len(datas)
		for _ = range dater.C {
			WorkTime()
			data := Data2String(",", DataProducter())
			datas[count] = data
			count += 1
			count %= length
		}
